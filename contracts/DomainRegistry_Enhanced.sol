...
contract DomainRegistry is ERC721URIStorageUpgradeable, AccessControlUpgradeable, IERC2981Upgradeable {
    ...
    mapping(uint256 => string) public customImageURI;
    event CustomImageUpdated(uint256 indexed tokenId, string newURI);

    function setCustomImageURI(uint256 tokenId, string calldata uri) external {
        require(_exists(tokenId), "Nonexistent token");
        address owner = ownerOf(tokenId);
        require(msg.sender == owner || hasRole(ADMIN_ROLE, msg.sender), "Not authorized");
        customImageURI[tokenId] = uri;
        emit CustomImageUpdated(tokenId, uri);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Nonexistent token");

        string memory name = string(abi.encodePacked("alsania", tokenId.toString(), ".fx"));

        if (bytes(customImageURI[tokenId]).length > 0) {
            return string(abi.encodePacked(
                "data:application/json;base64,",
                Base64.encode(bytes(string(abi.encodePacked(
                    '{"name":"', name,
                    '", "description":"An Enhanced Alsania Subdomain NFT.", "image":"', customImageURI[tokenId], '"}'
                ))))
            ));
        }
        ...
        for (uint8 i = 0; i < 5; i++) {
            uint256 x = (seed >> (i * 48)) % 300;
            uint256 y = (seed >> (i * 32)) % 300;
            uint256 r = (seed >> (i * 16)) % 50 + 10;
            shapes = string(abi.encodePacked(
                shapes,
                "<circle cx='", Strings.toString(x),
                "' cy='", Strings.toString(y),
                "' r='", Strings.toString(r),
                "' fill='#39ff14' opacity='0.15'/>"
            ));
        }

        string memory svg = string(abi.encodePacked(
            "<svg xmlns='http://www.w3.org/2000/svg' width='350' height='350' style='background:#001f3f'>",
            shapes,
            "<circle cx='30' cy='30' r='15' fill='#39ff14'/>",
            "<text x='50%' y='180' font-size='20' fill='#39ff14' text-anchor='middle' font-family='Permanent Marker, monospace'>",
            name,
            "</text></svg>"
        ));
        ...
        return string(abi.encodePacked(
            ...
            Base64.encode(bytes(string(abi.encodePacked(
                '{"name":"', name,
                '", "description":"An Enhanced Alsania Subdomain NFT.", "image":"', image, '"}'
            ))))
        ));
    }
