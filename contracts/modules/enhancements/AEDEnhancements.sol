 ...
 */
abstract contract AEDEnhancements is Initializable, ModuleBase, IAEDEnhancements {
    // Events
    ...
    function initializeModule() external override initializer {}
    ...
    function unlockFeature(uint256 tokenId, uint256 feature) external payable override onlyTokenOwner(tokenId) {
        uint256 cost = (feature == FEATURE_SUBDOMAINS) ? SUBDOMAIN_UNLOCK_PRICE : 0;
        require(msg.value >= cost, "Insufficient payment");
        _setFeature(tokenId, feature);
    }

    function isFeatureUnlocked(uint256 tokenId, uint256 feature) external view override returns (bool) {
        return (s().domainFeatures[tokenId] & feature != 0);
    }

    function getEnhancementPrice(uint8 feature) external view returns (uint256) {
        return s().enhancementPrices[feature];
    }
    ...
    function initializeModule_Enhancements() public virtual onlyInitializing {
        // Initialization logic for Enhancements module (optional)
    }
}