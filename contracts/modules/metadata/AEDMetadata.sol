 ...
 */
abstract contract AEDMetadata is Initializable, ModuleBase, IAEDMetadata {
    ...
    function _hasRole(bytes32 role, address account) internal view returns (bool) {
        // This assumes a role mapping in AppStorage or via IAEDCore
        // For now, check s().admins for ADMIN_ROLE
    ...
    // Removed duplicate and unnecessary override functions for setProfileURI, setImageURI, getProfileURI, getImageURI.
    ...
    modifier onlyOwnerOrAdmin(uint256 tokenId) {
        require(
            msg.sender == ownerOf(tokenId) || _hasRole(ADMIN_ROLE, msg.sender),
            "Not authorized"
        );
        _;
    }
    ...
    {
        if (!_exists(tokenId)) revert NonexistentToken();
        receiver = s().feeCollector;
        royaltyAmount = (salePrice * s().royaltyBps) / 10000;
